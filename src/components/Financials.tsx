...
export default function Financials() {
  ...
  const [currentView, setCurrentView] = useState<'dashboard' | 'revenue' | 'expenses' | 'profit' | 'account' | 'incomeTax' | 'purchaseTax'>('dashboard');
  const [viewedAccountId, setViewedAccountId] = useState<string | null>(null);

  // Tax configuration states
  const [incomeTaxType, setIncomeTaxType] = useState<'vat' | 'corporate' | 'zakat'>('vat');
  const [purchaseTaxType, setPurchaseTaxType] = useState<'inclusive' | 'exclusive'>('inclusive');
  const [purchaseTaxFilter, setPurchaseTaxFilter] = useState<'approved' | 'all'>('approved');
  ...
  const purchaseTransactions = useMemo(() => {
    return generalTransactions.filter(t => t.type === 'purchase').map(t => ({
      ...t,
      dateOriginal: t.date?.toDate?.() || new Date(t.date),
      dateDisplay: t.date?.toDate?.()?.toLocaleString('ar-SA') || new Date(t.date).toLocaleString('ar-SA')
    }));
  ...
  const stats = useMemo(() => {
    ...
    const filteredPurchases = purchaseTransactions.filter(t => 
      purchaseTaxFilter === 'all' ? true : t.status === 'approved'
    );
    const purchasesTotal = filteredPurchases.reduce((acc, t) => acc + (t.amount || 0), 0);

    const purchaseTax = purchaseTaxType === 'inclusive' 
      ? (purchasesTotal * 0.15) / 1.15 
      : purchasesTotal * 0.15;

    const netProfit = income - expense;
    let incomeTax = 0;
    if (incomeTaxType === 'vat') {
      incomeTax = income * 0.15;
    } else if (incomeTaxType === 'corporate') {
      incomeTax = Math.max(0, netProfit) * 0.20;
    } else if (incomeTaxType === 'zakat') {
      incomeTax = Math.max(0, netProfit) * 0.025;
    }
  ...
  }, [transactions, purchaseTransactions, incomeTaxType, purchaseTaxType, purchaseTaxFilter]);
  ...
  return (
    currentView !== 'dashboard' ? (
      <div className="space-y-6">
        ...
        {currentView === 'incomeTax' && (
          <div className="w-full space-y-8 pb-32">
            <DetailHeader 
              title={
                incomeTaxType === 'vat' 
                  ? "تفاصيل ضريبة القيمة المضافة (المخرجات)" 
                  : incomeTaxType === 'corporate' 
                  ? "تفاصيل ضريبة الدخل المستحقة (20%)" 
                  : "تفاصيل الزكاة الشرعية المستحقة (2.5%)"
              } 
              icon={Building2} 
              onBack={() => setCurrentView('dashboard')} 
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <Card className="md:col-span-1 rounded-3xl bg-slate-900 text-white border-none shadow-xl shadow-slate-950/20 p-8 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute -right-6 -bottom-6 w-32 h-32 bg-white/5 rounded-full blur-2xl pointer-events-none" />
                <p className="text-xs font-black opacity-60 uppercase tracking-widest mb-1">
                  {incomeTaxType === 'vat' ? 'إجمالي ضريبة المخرجات' : incomeTaxType === 'corporate' ? 'ضريبة الأرباح المقدرة' : 'الزكاة المستحقة'}
                </p>
                <h3 className="text-3xl font-black">{stats.incomeTax.toLocaleString()} <span className="text-xs font-normal">ر.س</span></h3>
                <p className="text-[9px] text-slate-400 mt-2 font-bold">
                  {incomeTaxType === 'vat' ? '15% من إجمالي الإيرادات' : incomeTaxType === 'corporate' ? '20% من صافي الربح' : '2.5% من صافي الربح'}
                </p>
              </Card>
              
              <Card className="md:col-span-3 rounded-3xl border-none shadow-sm bg-white p-6">
                <h4 className="font-bold mb-4 text-slate-800">بيانات الوعاء الضريبي</h4>
                <div className="grid grid-cols-3 gap-4 text-center mt-4">
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className="text-[10px] text-slate-400 font-bold">إجمالي الإيرادات</p>
                    <p className="text-lg font-black text-slate-700">{stats.income.toLocaleString()} ر.س</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className="text-[10px] text-slate-400 font-bold">صافي الربح الحالي</p>
                    <p className={`text-lg font-black ${stats.net >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{stats.net.toLocaleString()} ر.س</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl border border-primary/10 bg-primary/5">
                    <p className="text-[10px] text-primary font-bold">قيمة الضريبة المقدرة</p>
                    <p className="text-lg font-black text-primary">{stats.incomeTax.toLocaleString()} ر.س</p>
                  </div>
                </div>
              </Card>
            </div>
            ...
            <Card className="rounded-3xl border-none shadow-sm overflow-hidden">
              <div className="p-6 border-b bg-slate-50/50 flex justify-between items-center">
                <h4 className="font-bold">سجل العمليات الخاضعة للضريبة/الزكاة</h4>
              </div>
              <Table className="text-right" dir="rtl">
                <TableHeader className="bg-slate-50/50">
                  <TableRow>
                    ...
                    <TableHead className="text-right">النوع</TableHead>
                    <TableHead className="text-right">المبلغ</TableHead>
                    <TableHead className="text-right">الضريبة المقدرة (15%)</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {transactions.filter(t => t.unifiedType === 'income').map(tx => (
                    <TableRow key={tx.id}>
                      <TableCell className="font-medium text-[11px]">{tx.dateDisplay}</TableCell>
                      <TableCell className="font-bold text-slate-800">{tx.description}</TableCell>
                      <TableCell className="text-xs text-slate-500">إيرادات</TableCell>
                      <TableCell className="font-bold text-slate-800">{tx.amount.toLocaleString()} ر.س</TableCell>
                      <TableCell className="font-black text-emerald-600">{(tx.amount * 0.15).toLocaleString()} ر.س</TableCell>
                    </TableRow>
                  ))}
                  {transactions.filter(t => t.unifiedType === 'income').length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-20 text-slate-400">لا توجد إيرادات مسجلة</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
        ...
        {currentView === 'purchaseTax' && (
          <div className="w-full space-y-8 pb-32">
            <DetailHeader title="تفاصيل ضريبة المشتريات المجمعة" icon={CreditCard} onBack={() => setCurrentView('dashboard')} />
            
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <Card className="md:col-span-1 rounded-3xl bg-blue-600 text-white border-none shadow-xl shadow-blue-900/20 p-8 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute -right-6 -bottom-6 w-32 h-32 bg-white/5 rounded-full blur-2xl pointer-events-none" />
                <p className="text-xs font-black opacity-60 uppercase tracking-widest mb-1">إجمالي ضريبة المشتريات</p>
                <h3 className="text-3xl font-black">{stats.purchaseTax.toLocaleString()} <span className="text-xs font-normal">ر.س</span></h3>
                <p className="text-[9px] text-blue-100 mt-2 font-bold">
                  {purchaseTaxType === 'inclusive' ? '15% شاملة الضريبة (15/115)' : '15% مضافة على السعر الصافي'}
                </p>
              </Card>
              
              <Card className="md:col-span-3 rounded-3xl border-none shadow-sm bg-white p-6">
                <h4 className="font-bold mb-4 text-slate-800">بيانات المشتريات الإجمالية</h4>
                <div className="grid grid-cols-3 gap-4 text-center mt-4">
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className="text-[10px] text-slate-400 font-bold">مجموع الفواتير المحددة</p>
                    <p className="text-lg font-black text-slate-700">{stats.purchasesTotal.toLocaleString()} ر.س</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className="text-[10px] text-slate-400 font-bold">الحساب الضريبي</p>
                    <p className="text-sm font-black text-slate-600">{purchaseTaxType === 'inclusive' ? 'شامل الضريبة (15/115)' : 'غير شامل (15% إضافية)'}</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl border border-blue-100 bg-blue-50/50">
                    <p className="text-[10px] text-blue-600 font-bold">الضريبة المجمعة</p>
                    <p className="text-lg font-black text-blue-600">{stats.purchaseTax.toLocaleString()} ر.س</p>
                  </div>
                </div>
              </Card>
            </div>
            ...
            <Card className="rounded-3xl border-none shadow-sm overflow-hidden">
              <div className="p-6 border-b bg-slate-50/50 flex justify-between items-center">
                <h4 className="font-bold">سجل فواتير المشتريات والتكاليف الخاضعة للضريبة</h4>
              </div>
              <Table className="text-right" dir="rtl">
                <TableHeader className="bg-slate-50/50">
                  <TableRow>
                    <TableHead className="text-right">التاريخ</TableHead>
                    <TableHead className="text-right">البيان/المورد</TableHead>
                    <TableHead className="text-right">الحالة</TableHead>
                    <TableHead className="text-right">قيمة الفاتورة</TableHead>
                    <TableHead className="text-right">مبلغ الضريبة المقدر</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {purchaseTransactions.filter(t => purchaseTaxFilter === 'all' ? true : t.status === 'approved').map(tx => {
                    const txTax = purchaseTaxType === 'inclusive' 
                      ? (tx.amount * 0.15) / 1.15 
                      : tx.amount * 0.15;
                    return (
                      <TableRow key={tx.id}>
                        <TableCell className="font-medium text-[11px]">{tx.dateDisplay || new Date(tx.date).toLocaleDateString('ar-SA')}</TableCell>
                        <TableCell className="font-bold text-slate-800 text-right">
                          <div className="flex flex-col">
                            <span>{tx.description}</span>
                            {tx.attachmentUrl && (
                              <span className="text-[9px] text-blue-500 font-bold mt-1">📎 تحتوي على مرفق ممسوح</span>
                            )}
                          </div>
                        </TableCell>
                        <TableCell className="text-xs text-right">
                          <Badge className={tx.status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-none' : 'bg-amber-50 text-amber-700 border-none'}>
                            {tx.status === 'approved' ? 'مكتمل' : 'قيد الانتظار'}
                          </Badge>
                        </TableCell>
                        <TableCell className="font-bold text-slate-800">{tx.amount.toLocaleString()} ر.س</TableCell>
                        <TableCell className="font-black text-blue-600">{txTax.toLocaleString()} ر.س</TableCell>
                      </TableRow>
                    );
                  })}
                  {purchaseTransactions.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-20 text-slate-400">لا توجد فواتير مشتريات مسجلة</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
        ...
        {isManager && <StatCard title="صافي الربح" value={stats.net} icon={Wallet} color={stats.net >= 0 ? "text-emerald-600" : "text-red-600"} onClick={() => setCurrentView('profit')} />}
        
        {isManager && (
          <Card 
            onClick={() => setCurrentView('incomeTax')}
            className="rounded-2xl border bg-white shadow-sm p-3 md:p-5 overflow-hidden transition-all hover:shadow-md cursor-pointer group active:scale-95 flex flex-col justify-between"
          >
            <div className="flex flex-col h-full justify-between gap-2">
              <div className="flex items-start justify-between gap-1">
                <div className="min-w-0 text-right flex-1">
                  <p className="text-[10px] md:text-[11px] font-bold text-muted-foreground mb-1 uppercase tracking-wider truncate">
                    {incomeTaxType === 'vat' ? 'ضريبة المبيعات (15%)' : incomeTaxType === 'corporate' ? 'ضريبة الدخل (20%)' : 'الزكاة الشرعية (2.5%)'}
                  </p>
                  <h3 className="text-base md:text-xl lg:text-2xl font-black truncate text-slate-800 mt-1">
                    {stats.incomeTax.toLocaleString()} <span className="text-[10px] md:text-sm font-normal text-muted-foreground mr-1">ر.س</span>
                  </h3>
                </div>
                <div className="p-1.5 md:p-2 rounded-lg bg-slate-50 transition-colors group-hover:bg-primary group-hover:text-white text-slate-500 shrink-0 self-start mr-1">
                  <Building2 className="w-4 h-4 md:w-5 md:h-5" />
                </div>
              </div>

              {/* Interactive Toggle Switch */}
              <div className="flex items-center gap-1 bg-slate-50 p-0.5 rounded-lg border border-slate-100 self-end mt-1" onClick={(e) => e.stopPropagation()}>
                <button 
                  onClick={() => setIncomeTaxType('vat')} 
                  className={`text-[8px] md:text-[9px] font-black px-1.5 py-0.5 rounded-md transition-all ${incomeTaxType === 'vat' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                  title="ضريبة قيمة مضافة 15% على الإيرادات"
                >
                  ق.م 15%
                </button>
                <button 
                  onClick={() => setIncomeTaxType('corporate')} 
                  className={`text-[8px] md:text-[9px] font-black px-1.5 py-0.5 rounded-md transition-all ${incomeTaxType === 'corporate' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                  title="ضريبة دخل شركات 20% على الأرباح"
                >
                  دخل 20%
                </button>
                <button 
                  onClick={() => setIncomeTaxType('zakat')} 
                  className={`text-[8px] md:text-[9px] font-black px-1.5 py-0.5 rounded-md transition-all ${incomeTaxType === 'zakat' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                  title="زكاة شرعية 2.5% على الأرباح"
                >
                  زكاة 2.5%
                </button>
              </div>
        ...
        {isManager && (
          <Card 
            onClick={() => setCurrentView('purchaseTax')}
            className="rounded-2xl border bg-white shadow-sm p-3 md:p-5 overflow-hidden transition-all hover:shadow-md cursor-pointer group active:scale-95 flex flex-col justify-between"
          >
            <div className="flex flex-col h-full justify-between gap-2">
              <div className="flex items-start justify-between gap-1">
                <div className="min-w-0 text-right flex-1">
                  ...
                  <h3 className="text-base md:text-xl lg:text-2xl font-black truncate text-blue-600 mt-1">
                    {stats.purchaseTax.toLocaleString()} <span className="text-[10px] md:text-sm font-normal text-muted-foreground mr-1">ر.س</span>
                  </h3>
                </div>
                <div className="p-1.5 md:p-2 rounded-lg bg-slate-50 transition-colors group-hover:bg-primary group-hover:text-white text-blue-500 shrink-0 self-start mr-1">
                  <CreditCard className="w-4 h-4 md:w-5 md:h-5" />
                </div>
              </div>

              {/* Interactive Toggles */}
              <div className="flex items-center justify-between w-full mt-1 border-t border-slate-55 pt-1.5" onClick={(e) => e.stopPropagation()}>
                {/* State/Type Toggle */}
                <div className="flex items-center gap-0.5 bg-slate-50 p-0.5 rounded-md border border-slate-100">
                  <button 
                    onClick={() => setPurchaseTaxType('inclusive')} 
                    className={`text-[8px] font-black px-1.5 py-0.5 rounded-md transition-all ${purchaseTaxType === 'inclusive' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                    title="شامل الضريبة 15% (15/115)"
                  >
                    شامل
                  </button>
                  <button 
                    onClick={() => setPurchaseTaxType('exclusive')} 
                    className={`text-[8px] font-black px-1.5 py-0.5 rounded-md transition-all ${purchaseTaxType === 'exclusive' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                    title="غير شامل الضريبة 15% (15% إضافية)"
                  >
                    صافي
                  </button>
                </div>
                ...
                <div className="flex items-center gap-0.5 bg-slate-50 p-0.5 rounded-md border border-slate-100">
                  <button 
                    onClick={() => setPurchaseTaxFilter('approved')} 
                    className={`text-[8px] font-black px-1 py-0.5 rounded-md transition-all ${purchaseTaxFilter === 'approved' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                    title="الفواتير المعتمدة فقط"
                  >
                    معتمد
                  </button>
                  <button 
                    onClick={() => setPurchaseTaxFilter('all')} 
                    className={`text-[8px] font-black px-1 py-0.5 rounded-md transition-all ${purchaseTaxFilter === 'all' ? 'bg-primary text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                    title="جميع الفواتير المضافة"
                  >
                    الكل
                  </button>
                </div>
